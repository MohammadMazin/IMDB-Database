/*======================================================================================================
 *          IMDB Mid Semester Project
 *          Data Structures And Algorithms
 *          Submitted to: Dr Yasir Faheem
 *          Submitted by: Syed Mohammad Mazin(306913), Umar Hassan Khan(288929)
 *          Section: BSCS-9B
 * =====================================================================================================
*/
...
public:
    ...
    void SearchMovieRating(float score) {
        ...
        while (loc != NULL && (loc->pmData->imdbScore < score || loc->pmData->imdbScore == score)) {         //loop runs till value is found while comparing IMDB score
            ploc = loc;
            loc = loc->next;
        }
        ...
        if(loc != NULL){ //if currLoc is not null, means value already exists in list
            ...
            if(ploc == NULL){ //if predloc and currLoc both null, means position is found and list is empty so value will be added to front of list
                ...
                if(ploc == last){ //if predLoc has last node address, value will be inserted to the end
                    ...
                    MovieNode *newNode = new MovieNode();
                    newNode->pmData=md;
//                    newNode -> data.actorName = ac.actorName;
//                    newNode -> data.actorFacebookLikes = ac.actorFacebookLikes;
    ...
    void SearchMovieYear(int year) {
        ...
        while (loc != NULL && (loc->pmData->titleYear < year || loc->pmData->titleYear == year)) {   //loop runs till value is found
            ploc = loc;
            loc = loc->next;
        }
    ...
    void InsertSortedMoviesYear(MovieData *md){
        ...
        SearchMovieYear(md->titleYear);		 //Title Year of Movie is compared to nodes present in movieList to find the node's logical position
        if(loc != NULL){ 					 //if currLoc is not null, means value already exists in list
            return;
        }
        else{ 								 // if currLoc is null, then..
...
///=============Forward Declared Functions==============
    ...
    void PrintMoviesRatingWise(bool callFromOtherFunc);
    ...
    void PrintMoviesOfGenre(string genre, MovieNode *md);
    ...
    void PrintDirectorList(){
        ...
        if(!DirectorListIsEmpty()){
            ...
            while(temp != NULL){	 //looping till temp is not null
                cout <<" Director Node: "<<i<<" "<<temp->pData->directorName<<" got likes: "<<temp -> pData->directorFacebookLikes<<endl; //print Director's name and likes
                temp = temp -> next;
                i++;
            }
        }
    ...
    void PrintDirectorListAndTheMoviesTheyDirected(){
        ...
        if(!DirectorListIsEmpty()){
            ...
            while(temp != NULL){		//looping till temp is not null
                ...
                //print director info
                cout <<" Director Node: "<<i<<" "<<temp->pData->directorName<<" got likes: "<<temp -> pData->directorFacebookLikes<<".\n Directed ";
...
void ActorList::PrintActorList(){
    ...
    if(!ActorListIsEmpty()){
        ...
        while(temp != NULL){ 	 //looping till temp is not null
            cout <<"Actor Node: "<<i<<": "<<temp->paData->actorName<<"\t\t got likes: "<<temp->paData->actorFacebookLikes<<endl;  //print actor name and facebook likes
            temp = temp -> next;
            i++;
        }
    }
...
void ActorList::SearchActor(ActorData *obj) {
    ...
    while (loc != NULL && loc->paData->actorName < obj->actorName) {         //loop runs till value is found. Names of actors are compared
        ploc = loc;
        loc = loc->next;
    }
...
void ActorList::AreBothCoActors(string act1, string act2){
    ...
    if(act1 < act2){						   //comparing strings || alphabetically smaller string is searched for in Actor list
        ...
        while(loc!= NULL){					   //searches for actor1
            if (((upper_string(loc->paData->actorName).find(act1Upper) != string::npos))){	//executes if name of actor is found in list
                ...
                while(coAct != NULL){		  					//searches for co-actors name
                    ...
                    if ((upper_string(coAct->paData->actorName).find(act2Upper) != string::npos)){ //executes if name of co-actor is found in list

                        act2Name = coAct->paData->actorName;    //co actor name saved to string
                        cout<<loc->paData->actorName<<" and "<<coAct->paData->actorName<< " are Co-Actors!"<<endl;   //printing actor and co actor names
                        areCoActors = true;
                        break;
                    }
            ...
            if (((upper_string(loc->paData->actorName)).find(act2Upper) != string::npos)) {
                ...
                while (coAct != NULL) {
                    if (((upper_string(coAct->paData->actorName)).find(act1Upper) != string::npos)) {
                        ...
                        cout << loc->paData->actorName << " and " << coAct->paData->actorName << " are Co-Actors!" << endl;
                        areCoActors = true;
                        break;
                    }
                    coAct = coAct->next; //move to next node
    ...
    if(!areCoActors){
        ...
        while(ml.loc != NULL){							   //traversing through movie list
            if((ml.loc->pmData->actors[0]->actorName == act1Name || ml.loc->pmData->actors[1]->actorName == act1Name || ml.loc->pmData->actors[2]->actorName == act1Name)
               && (ml.loc->pmData->actors[0]->actorName == act2Name || ml.loc->pmData->actors[1]->actorName == act2Name || ml.loc->pmData->actors[2]->actorName == act2Name)){   //executes if a movie data object contains names of both actor1 and actor2
...
void ActorList::SearchCoActorOfCoActor(string aname){
    ...
    while(loc != NULL) {
        if (upper_string(loc->paData->actorName) == upName) {   //name is searched for in ActorList
            ...
            loc->paData->MoviesActedIn->SearchCoActorCoActorMovieListlevel2(upName,upName);  //Function call to find coActors
            return;
        }
        loc= loc->next;
    }
...
// data from row is stored into array
//data from array is stored into MovieData objects
//Actors , CoActors, DirectorList, ActorList is also filled in this function

//Big O complexity = N^3
//Big Omega complexity = N^3
...
void PopulateStuff(string fileName){
    ...
    while(myFile.good()){
        ...
        dl.InsertSortedDirectors(tempD,tempm);			//movie created by director and director named added to respective lists
...
void MovieList::PrintActorsOfMovie(string name){
    ...
    while(loc != NULL) {
        ...
        if (upper_string(loc->pmData->movieTitle) == upper_string(name)) {   //uppercase strings are compared and executes if names match
            cout << "Movie: " << loc->pmData->movieTitle << ".\t Actor1: " << loc->pmData->actors[0]->actorName <<" || Actor2: " << loc->pmData->actors[1]->actorName << " || Actor3: " << loc->pmData->actors[2]->actorName << endl;
            return;
        }
        loc= loc->next;			//move to next node
    }
...
void MovieList::PrintMovieList(){
    ...
    if(!MovieListIsEmpty()){
        ...
        while(temp != NULL){ //looping till temp reaches end of list
            ...
            cout <<"Movie "<<i<<": "<<temp -> pmData->movieTitle<<" || Released in "<<temp->pmData->titleYear<<endl;
...
void MovieList::PrintMovieListInDetail(){
    ...
    if(!MovieListIsEmpty()){
        ...
        temp = start;					//initialize the temp
        while(temp != NULL){			//looping till temp is not null
            cout <<"Movie Node: "<<i<<": "<<temp -> pmData->movieTitle<<". Which was realeased in "<<temp->pmData->titleYear<<endl;
            temp = temp -> next;
            i++;
        }
    }
...
void MovieList::SearchCoActorCoActorMovieListlevel2(string ss,string sx){
    ...
    if(!MovieListIsEmpty()){
        ...
        while(temp != NULL){ //looping till temp is not null
            if(ss != upper_string(temp->pmData->actors[0]->actorName)){
                temp->pmData->actors[0]->MoviesActedIn->PrintCoActorssF4(upper_string(temp->pmData->actors[0]->actorName),sx);
            }
            if(ss != upper_string(temp->pmData->actors[1]->actorName)){
                temp->pmData->actors[1]->MoviesActedIn->PrintCoActorssF4(upper_string(temp->pmData->actors[1]->actorName),sx);
            }
            if(ss != upper_string(temp->pmData->actors[2]->actorName)){
                temp->pmData->actors[2]->MoviesActedIn->PrintCoActorssF4(upper_string(temp->pmData->actors[2]->actorName),sx);
            }
            temp = temp -> next;
            i++;
        }
...
//the boolean function determines if this function was called from PrintMoviesofGenre()
//in that case, this function exits after populating the RatingList

//BigO time complexity = N^2
//Big Omega time Complexity N

void MovieList::PrintMoviesRatingWise(bool calledFromOtherFunc){
    bool isRatingListEmpty = mlRating.MovieListIsEmpty();           //stores bool value if RatingList is completely filled
    if(MovieListIsEmpty()){
        cout<< "Movie List is Empty. Cannot sort Rating Wise";      //if Original Movielist is empty, this function exits
        return;
    }
    ...
    if(isRatingListEmpty){                                          //if RatingList is empty, Movie inserted at front of that list
        ...
        ploc = loc;
        loc = loc->next;                                            //changing pointer locations
    }
    while(loc!= NULL  && isRatingListEmpty){                        // executes if RatingList is not completely filled or not and traverses ent

        mlRating.InsertSortedMoviesRating(loc->pmData);             //value is sorted based on IMDB rating

        loc = loc->next;                                            //moving to next node in MovieList


    }

    if(calledFromOtherFunc)
        return;

    mlRating.loc = mlRating.start->next;
    int i=1;                                                        //Count Variable
    while(mlRating.loc!= NULL) {                                    //Traverse till end of RatingList and print MovieTitle and IMDB score
        cout << "Movie #"<<i<<": " << mlRating.loc->pmData->movieTitle << " || Rating: " << mlRating.loc->pmData->imdbScore
             << endl;
        mlRating.loc = mlRating.loc->next;                          //move to next node in RatingList
        i++;
    }

    isRatingListEmpty = mlRating.MovieListIsEmpty();                //bool stores if list is full or not
}
...
void MovieList::PrintMoviesYearWise(bool printInAscendingYears) {
    ...
    while(loc!= NULL && isYearListEmpty){						// executes if YearList is not completely filled or not and traverses entire MovieList while sorting based on year
    ...
    if(mlYear.printAscendingYears)
        while(mlYear.loc!= NULL) {								//executes if TRUE - movies printed in ascending order
...
void MovieList::PrintDetailsOfMovie(string name){
    ...
    while(loc!=NULL){						//traverses through movie list
        ...
        if(upper_string(loc->pmData->movieTitle).find(uname) != string::npos){ //if movie name matches, all of its details are printed
            valFound = true;
            ...
            cout<<"Movie: "<<loc->pmData->movieTitle<<endl
                ...
                <<"Actor #1 : "<<loc->pmData->actors[0]->actorName<<" || Facebook Likes: "<<loc->pmData->actors[0]->actorFacebookLikes<<endl
                <<"Actor #2 : "<<loc->pmData->actors[1]->actorName<<" || Facebook Likes: "<<loc->pmData->actors[1]->actorFacebookLikes<<endl
                <<"Actor #3 : "<<loc->pmData->actors[2]->actorName<<" || Facebook Likes: "<<loc->pmData->actors[2]->actorFacebookLikes<<endl
...
void MovieList::PrintDirectorsOfGenre(string genre) {
    ...
    while (dl.loc != NULL) {                                //loop goes through Director List
        ...
        while(movData!= NULL) {                                             //loop goes through MoviesDirectedList of Current DirectorNode

            if (((upper_string(movData->pmData->genres)).find(genre_Upper) != string::npos)) {          //executes if genre matches in one of the direted movies
...
//Prints Movies of a genre from high rating to low
//checks if RatingList is filled or not
//if not , it fills it and then calls recursive function
//functions traverses to end of RatingList and then matches genre with user input
//if both match, the movie is printed

//big O time complexity N^2
//Big Omega time complexity N
void MovieList::PrintMoviesOfGenre(string genre, MovieNode *md ){

    if(mlRating.MovieListIsEmpty()){                     //checks ratingList
        ml.PrintMoviesRatingWise(true);
        md = mlRating.start;                             //md Node is set

    }
    if(md != NULL){		//Base Case - executes till last node of list has been crossed

        PrintMoviesOfGenre(upper_string(genre), md->next);  //moving nodes
        if ((upper_string(md->pmData->genres)).find(genre) != string::npos) {   //searches for string in genre of MovieData

            cout << "Movie: " << md->pmData->movieTitle << " || Rating: "           //prints movieTitle , Rating and Genres
                 << md->pmData->imdbScore<<" || Genre: "<<md->pmData->genres<<endl;
        }
    }
    ...
    while(true){								//text user interface to perform different functions;
        ...
        switch (userNumInput) {
            ...
            case 9:
                ...
                break;
            case 10:
                cout<<"Press '1' to see movies released, in ascending order. Press '2' to see movies released, in descending order: ";
                cin>>userNumInput;
                if(userNumInput == 1)
            ...
            case 11:
                ...
                cin.ignore();
                getline(cin,userStringInput);
                ml.PrintMoviesOfGenre(upper_string(userStringInput), mlRating.start);
                break;
            case 12:
                cout<<"Movies Sorted Rating Wise!"<<endl;
                ml.PrintMoviesRatingWise(false);
                break;
